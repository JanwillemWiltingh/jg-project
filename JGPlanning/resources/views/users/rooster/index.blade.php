...
<div class="content fadeInDown">
                                        ...
                                        @for($i = 0; $i < count($days); $i++)
                                            @if(is_array($days[$i]))
                                                <th rowspan="{{ $days[$i]['rowspan'] }}" class="align-middle text-center" style="@if($days[$i]['start_time'] != "") background-color: #1C88A4; @else background-color:#f0f0f0; @endif color: white;">
                                                    ...
                                                    @if(!$days[$i]['comment'] == "")
                                                        @if($days[$i]['comment'] == "Disabled")
                                                            @if($days[$i]['by_admin'] == 0)
                                                                <a href="#" data-bs-toggle="modal" data-bs-target="#editDisableModal" style="font-weight: lighter; text-decoration: none;" onclick="modalData({{$days[$i]['id']}})"><i class="fa fa-pencil-alt"></i></a>
                                                    ...
                                                    @endif
                                                    @if($days[$i]['start_time'] != "")
                                                        <p style="font-weight: lighter">{{$days[$i]['start_time']}} - {{$days[$i]['end_time']}}</p> <a href="#" data-bs-toggle="modal" data-bs-target="#availabilityModalEdit" style="font-weight: lighter; text-decoration: none; font-size: 15px; color: white; " onclick="modalData({{$i + 1}}, {{$days[$i]['id']}})"><i class="fa fa-pencil-alt"></i></a>
                                                    @endif
                                                </th>
                                            @elseif ($days[$i] === 1)
                                                <td></td>
    ...
    <script>
        function modalData(weekday, id)
        {
            ...
            // document.getElementById('weekday').value = Var;
            // document.getElementById('weekday_edit').value = Var;
